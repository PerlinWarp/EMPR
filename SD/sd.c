 ...
 * @file                SSP_sdcard.c
 * @purpose             This example describes how to use SSP at mode SSP master/8bit
...
"SSP communicate with SD card demo \n\r"
"\t - MCU: LPC17xx \n\r"
"\t - Core: ARM Cortex-M3 \n\r"
"\t - Communicate via: UART0 - 115200bps \n\r"
" Demo SSP module, read SD card's CID register and display via UART0\n\r"
"********************************************************************************\n\r";
...
// _DBG(" status = ");
// _DBD(xferConfig.status);
// _DBG(" Return Val ");
// _DBD(xferConfig.counter);
// _DBG("\n\r");
     // for(i=0;i<1000;i++);
        // CS_Force(1);
        return xferConfig.rx_cnt;
}
/*********************************************************************/
void SD_SendCommand(uint8_t cmd, uint8_t *arg)
{
...
//      while(SD_GetCardConnectStatus()==SD_DISCONNECTED);
        // _DBG("...Connected!\n\r");
        // Wait for bus idle
        if(SD_WaitDeviceIdle(1600) != SD_OK) return SD_ERROR_BUS_NOT_IDLE;
        _DBG("Initialize SD card in SSP mode...");

        errors = 0;
        /* Send the CMD0_GO_IDLE_STATE while CS is asserted */
        /* This signals the SD card to fall back to SSP mode */
...
* @return               None
*******************************************************************************/
void check_failed_pcc(uint8_t *file, uint32_t line)
{
